...
> **请务必使用最新版本。** 当前版本：**v0.1.17**（见 [`VERSION`](VERSION)）。运行下方安装脚本后，终端会输出实际使用的版本号。
>
...
备用地址：`https://github.com/gsvps/GSNode/raw/main/cleanup.sh`、`https://cdn.jsdelivr.net/gh/gsvps/GSNode@main/cleanup.sh`

跳过确认：

```bash
curl -fsSL https://dl.gsvps.com/cleanup.sh | sh -s -- -y
```

仅预览（不删除）：

```bash
curl -fsSL https://dl.gsvps.com/cleanup.sh | sh -s -- -n
```
